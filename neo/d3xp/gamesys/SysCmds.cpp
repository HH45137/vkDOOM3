...
void Cmd_Give_f( const idCmdArgs &args ) {
	...
	if ( idStr::Icmp( name, "pda" ) == 0 ) {
		if ( args.Argc() == 2 ) {
			...
			player->GivePDA( NULL, NULL );
			for ( i = 0; i < declManager->GetNumDecls( DECL_PDA ); i++ ) {
...
void Cmd_SetActorState_f( const idCmdArgs &args ) {
	...
	if ( args.Argc() != 3 ) {
		idLib::Printf( "usage: setActorState <entity name> <state>\n" );
		return;
	}
...
void Cmd_TestId_f( const idCmdArgs &args ) {
	...
	int		i;
	if ( args.Argc() == 1 ) {
		idLib::Printf( "usage: testid <string id>\n" );
		return;
	}
...
===============
*/
void Cmd_HideWeapon_f( const idCmdArgs & args ) {
	gameLocal.world->spawnArgs.SetBool( "no_Weapons", true );
	idPlayer * player = gameLocal.GetLocalPlayer();
...
===============
*/
void Cmd_ShowWeapon_f( const idCmdArgs & args ) {
	gameLocal.world->spawnArgs.SetBool( "no_Weapons", false );
	idPlayer * player = gameLocal.GetLocalPlayer();
...
//	cmdSystem->AddCommand( "clientCallVote",		idMultiplayerGame::CallVote_f,	CMD_FL_GAME,			"call a vote: clientCallVote si_.. proposed_value" );
	...
	cmdSystem->AddCommand( "HideWeapon",			Cmd_HideWeapon_f,			CMD_FL_GAME,				"" );
	cmdSystem->AddCommand( "ShowWeapon",			Cmd_ShowWeapon_f,			CMD_FL_GAME,				"" );
}
